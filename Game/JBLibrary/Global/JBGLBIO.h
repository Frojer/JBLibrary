...
namespace JBL{
    namespace IO{
        ...
        /// @param self 대상 바이트
        /// @return 뒤집힌 값
        extern __forceinline _BYTE glb_getReverseByte(const _BYTE self);
        ...
        public:
            ...
            /// @return 초기화 성공 여부
            __forceinline bool init(const bool isRead, const wchar_t* fileName){
            ...
            /// @return 초기화 성공 여부
            __forceinline bool init(const bool isRead, const char* fileName){
                ins_state.read = isRead;
        ...
        public:
            ...
            /// @return 현재 위치(4바이트)
            __forceinline long getCurPos(){ return ftell(ins_file); }
            /// @brief 현재 입출력 객체가 읽고 있는 파일의 위치를 구합니다.
            /// @return 현재 위치(8바이트)
            __forceinline __int64 getCurPosI64(){ return _ftelli64(ins_file); }
        public:
        ...
        public:
            ...
            /// @return 동작 성공 여부
            __forceinline bool moveForward(const long size){ return !fseek(ins_file, size, SEEK_SET); }
            /// @brief 맨 처음 위치로부터 입출력 객체가 읽을 위치를 설정합니다.
            /// @param size 위치 크기(8바이트)
            /// @return 동작 성공 여부
            __forceinline bool moveForwardI64(const __int64 size){ return !_fseeki64(ins_file, size, SEEK_SET); }
        public:
            /// @brief 맨 마지막 위치로부터 입출력 객체가 읽을 위치를 설정합니다.
            /// @param size 위치 크기(4바이트)
            /// @return 동작 성공 여부
            __forceinline bool moveBackward(const long size){ return !fseek(ins_file, size, SEEK_END); }
            /// @brief 맨 마지막 위치로부터 입출력 객체가 읽을 위치를 설정합니다.
            /// @param size 위치 크기(8바이트)
            /// @return 동작 성공 여부
            __forceinline bool moveBackwardI64(const __int64 size){ return !_fseeki64(ins_file, size, SEEK_END); }
        public:
            /// @brief 현재 위치로부터 입출력 객체가 읽을 위치를 설정합니다.
            /// @param size 위치 크기(4바이트)
            /// @return 동작 성공 여부
            __forceinline bool jump(const long size){ return !fseek(ins_file, size, SEEK_CUR); }
            /// @brief 현재 위치로부터 입출력 객체가 읽을 위치를 설정합니다.
            /// @param size 위치 크기(8바이트)
            /// @return 동작 성공 여부
            __forceinline bool jumpI64(const __int64 size){ return !_fseeki64(ins_file, size, SEEK_CUR); }
        public:
            /// @brief 현재 위치에 1바이트 크기만큼 데이터를 기록합니다.
            /// @param data 기록할 데이터
            /// @param isFlip 바이트 뒤집기 여부
            /// @return 동작 성공 여부
            __forceinline bool put(_BYTE data, const bool isFlip = false){ data = isFlip ? glb_getReverseByte(data) : data; return putc(data, ins_file) == data; }
            /// @brief 현재 위치로부터 1바이트 크기만큼 데이터를 읽어옵니다.
            /// @param isFlip 바이트 뒤집기 여부
            /// @return 읽어올 데이터(EOF일 경우 오류)
            __forceinline _INT get(const bool isFlip = false){ auto c = getc(ins_file); return c != EOF ? isFlip ? glb_getReverseByte(c) : c : EOF; }
        public:
