...
namespace JBL{
    ...
    public:
        ...
        /// @return 로그 핸들
        __forceinline HWND getHWND()const{ return ins_thisHwnd; }
    public:
        /// @brief 로그를 표시합니다.
        /// @param str 메세지(멀티바이트)
        /// @param attribute 메세지 구분(0: 일반, 1: 성공, 2: 경고, 3: 실패, 4: 정보)
        __forceinline void addLog(const _STRING& str, const char attribute){ return ins_pushLog(STRING::glb_ansiToWide(str.c_str()), attribute); }
        /// @brief 로그를 표시합니다.
        /// @param str 메세지(와이드바이트)
        /// @param attribute 메세지 구분(0: 일반, 1: 성공, 2: 경고, 3: 실패, 4: 정보)
        __forceinline void addLog(const _WSTRING& str, const char attribute){ return ins_pushLog(str, attribute); }
    private:
